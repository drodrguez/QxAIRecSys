...
def feature_matching_ratio_custom(feature_batch, test_feature):
    count = 0
...
class DataLoader:
    ...
    def load_data(self, data_path, index_dir):
        ...
        reviews = pd.read_csv(data_path)
        for _, review in reviews.iterrows():
            #(fea, adj, tem, sco) = review['feature']
            tem = review['review']
            fea = review['feature']
    ...
    def __init__(self, data, user2feature, item2feature, tokenizer, bos, eos, seq_len, batch_size=128, shuffle=False):
        ...
        for x in data:
          try:
            ...
            errors_in_batching += 1
            continue
        print(f'Total errors in batching: {errors_in_batching}')
        ...
        self.shuffle = shuffle
        self.batch_size = batch_size
        self.sample_num = len(data) - errors_in_batching
        self.index_list = list(range(self.sample_num))
        self.total_step = int(math.ceil(self.sample_num / self.batch_size))
        self.step = 0
