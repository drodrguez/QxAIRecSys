...
from utils import rouge_score, bleu_score, DataLoader, Batchify2, now_time, ids2tokens, unique_sentence_percent, \
    feature_detect, feature_matching_ratio, feature_coverage_ratio, feature_diversity, feature_matching_ratio_custom
import pickle
import numpy as np
...
nDIV = (1/mean_fui)*feature_diversity(feature_batch) 
...
for (feature_set_path, feature_dict_path, testset_path) in paths:
          ...
          FMR = feature_matching_ratio_custom(feature_batch, feature_test)
          print(now_time() + 'FMR {:7.4f}'.format(FMR))
